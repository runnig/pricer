...
void book_t::handle_message(const message_t & m)
{
	if (m.timestamp < 0)
	{
		throw invalid_timestamp(m.timestamp);
	}

	if (m.id < 0)
	{
		throw invalid_order_id(m.timestamp, m.id);
	}

	if ('A' == m.msg_type)
	{
		if ('B' != m.side && 'S' != m.side)
		{
			throw invalid_side(m.timestamp, m.side);
		}
		if (m.price <= 0)
		{
			throw invalid_price(m.timestamp, m.price);
		}
		if (m.size <= 0)
		{
			throw invalid_size(m.timestamp, m.size);
		}
