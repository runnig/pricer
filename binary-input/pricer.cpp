...
static void usage()
{
    std::cerr << "usage: pricer target-size max-messages pricer.in.bin\n";
}
...
struct order
{
    ...
    bool active() const { assert(m_size >= 0); return m_size > 0;}
    ...
    side_t side() const { return m_side; }
	price_t price() const { return m_price; }
...
static
bool if_deleted(const order_ptr & a) { return !a->active(); }

static
side_t opposite_side(side_t s)
{ 
	assert(BUY==s || SELL==s); 
...
struct market_base
{
            ...
            price_t price, int size)
	{
        ...
        {
            throw cant_add_order(timestamp, id, "order with the id already exists in the book");
        }
...
#ifndef PROFILING
                os << m_last_timestamp << ' ' << 
                    char_from_side(opposite_side(m_side)) << " NA\n";
#endif
	...
	bool has_order(order_id_t id) const { return m_orders.find(id)!= m_orders.end();}
...
{
	buy_market() : market_base(BUY) {}

	virtual void evaluate_income(int target_size, std::ostream &os)
	{
		if(not_available(target_size, os)) { return; }
...
struct book_t
{
    ...
    void add_order(timestamp_t timestamp, order_id_t id, 
            side_t side, price_t price, int size)
    {
        assert(BUY == side || SELL == side);
		m_last_side = side;
...
void book_t::handle_message(const message_t & m)
{
    if(m.timestamp < 0) { throw invalid_timestamp(m.timestamp); }

    if(m.id < 0) { throw invalid_order_id(m.timestamp,m.id); }

    if('A' == m.msg_type)
    {
        if('B' != m.side && 'S' != m.side) { throw invalid_side(m.timestamp, m.side); }
		if(m.price <= 0) { throw invalid_price(m.timestamp, m.price); }
        if(m.size <= 0) { throw invalid_size(m.timestamp, m.size); }

        side_t side = side_from_char(m.side);
        add_order(m.timestamp, m.id, side, m.price, m.size);
    }
    else if('R'== m.msg_type)
    {
        if(m.size <= 0) { throw invalid_size(m.timestamp, m.size); }

        reduce_order(m.timestamp, m.id, m.size);
    }
    else
    {
        throw unsupported_msg_type(m.timestamp, m.msg_type);
    }
...
int main(int argc, char* argv[])
{
	...
	{
		f.open(input_filename, std::ios_base::binary | std::ios_base::in);
		is = &f;
	}
	else
	{
    ...
    while(is->good())
    {
		...
		if(max_messages > 0 && num_line > max_messages) { break; }

		message_t m;
		memset(&m, 0, sizeof(m));
		is->read((char*)&m, sizeof(m));
		if(m.msg_type == 0) { break;}

		if(is->bad()) { break; }

        try{
            book.handle_message(m);
