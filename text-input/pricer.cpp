...
static
order_id_t order_id_from_str(const std::string & s)
{
...
void book_t::handle_message(const std::string & msg)
{
    ...
    if(timestamp < 0) { throw invalid_timestamp(timestamp); }

    id = order_id_from_str(id_str);
    if(id < 0) { throw invalid_order_id(timestamp,id); }
    ...
    if('A' == msg_type)
    {
        ...
        if('B' != ch && 'S' != ch) { throw invalid_side(timestamp, ch); }
		if(price <= 0) { throw invalid_price(timestamp, price); }
        if(size <= 0) { throw invalid_size(timestamp, size); }
